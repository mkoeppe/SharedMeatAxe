...
int WriteResult(int cf, int cond_dim)

{
    ...
    if (snprintf(fn,100,"%s%s.v",LI.BaseName,Lat_CfName(&LI,cf))>=100)
    {
        MTX_ERROR("Buffer overflow");
        return 1;
    }
    MESSAGE(1,("Writing %s\n",fn));
    if (MatSave(result,fn))
    {
        MatFree(result);
        return 1;
    }
...

{
    ...
    /* Read the generators and the condensed peak words
       ------------------------------------------------ */
    if (snprintf(fn,200,"%s%s.%%dk",LI.BaseName,Lat_CfName(&LI,cf))>=200)
    {
        MTX_ERROR("Buffer overflow");
        return 1;
    }
    ...
    if (snprintf(fn,200,"%s%s.np",LI.BaseName,Lat_CfName(&LI,cf))>=200)
    {
        MTX_ERROR("Buffer overflow");
        return 1;
    }
    if (MrAddGenerator(Rep,MatLoad(fn),0)) return 1;
    ...
    /* Clean up
       -------- */
    for (i = 0; i < NCyclic; ++i) MatFree(Cyclic[i]);
    MrFree(Rep);
    return 0;
}
...
@section mkcycl-syntax Command Line
<pre>
...
@section mkcycl-inp Input Files
@par @em Name.cfinfo
...
@section mkcycl-out Output Files
@par @em NameCf.v
  Cyclic submodules.

@section mkcycl-desc Description
...
@section mkcycl-impl Implementation Details
