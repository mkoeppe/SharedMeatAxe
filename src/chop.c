/* ============================= C MeatAxe ==================================
   File:        $Id: chop.c,v 1.1.1.1 2007/09/02 11:06:16 mringe Exp $
   Comment:     Chop a representation (Calculate composition series)
   --------------------------------------------------------------------------
   (C) Copyright 1999-2005 Michael Ringe, Lehrstuhl D fuer Mathematik,
...
/* --------------------------------------------------------------------------
   ...
   Return:
     1 = success (module was split), 0 = failed, -1 = error
...
*/
    ...
    if (!MatMul(MatMul(MatMul(MatMul(v,n->word),iA),B),iA))
        { MatFree(v1);
          MatFree(iA);
          MatFree(B);
          return -1;
        }
    if (!MatMul(MatMul(MatMul(MatMul(v1,iA),B),iA),B))
        { MatFree(v1);
          MatFree(iA);
          MatFree(B);
          return -1;
    ...
    if (!MatAddMul(v,v1,FfNeg(FF_ONE)))
        { MatFree(v1);
          MatFree(iA);
          MatFree(B);
          return -1;
