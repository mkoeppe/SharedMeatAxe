...
/* -----------------------------------------------------------------
   ReadFiles(), 1 on error, 0 on success
   ----------------------------------------------------------------- */
...

{
    ...
    /* Read the incidence matrix
       ------------------------- */
    if (snprintf(fn,200,"%s.inc",LI.BaseName)>=200)
    {
    ...
    if (SysReadLong(f,&nmountains,1)<0) return 1;
    ...
    /* Read classes
       ------------ */
    if (snprintf(fn,200,"%s.mnt",LI.BaseName)>=200)
    {
        MTX_ERROR("Buffer overflow");
        return 1;
    }
